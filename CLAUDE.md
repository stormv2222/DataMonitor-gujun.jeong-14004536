...
main.py  (Wiring)
  ...
  └── MonitorController →  controllers/monitor_controller.py [Controller]
          │
          ├── _repo  (RecordRepository)   ← CRUD 수행
...
- **Model** (`models/record.py`): `Record` dataclass(id 자동 부여) + `RecordRepository`(json_lib 기반 파일 저장). DataPersistence의 `Repository` 로직을 MVC Model로 재구성.
...
- **Controller** (`controllers/monitor_controller.py`): `__init__`에서 `RecordRepository`, `MonitorView`, `FileWatcher`를 주입받음. `run()`이 메뉴 루프 진입점(a/u/d/s/q). 파일 변경 감지 시 `view.show_dashboard()` 재호출로 자동 갱신.
...
- **데이터 파일 스키마 고정**: `{"next_id": int, "records": [{id, ...fields}]}` 구조 유지. `id`는 자동 부여이며 사용자가 직접 입력하지 않는다.
